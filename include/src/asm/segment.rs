...
#[inline]
fn get_fs_byte(addr: *const u8) -> u8 {
    let _v: u8;
...
#[inline]
fn get_fs_word(addr: *const u16) -> u16 {
    let _v: u16;
    unsafe { llvm_asm!("movb %%fs:%1,%0":"=r" (_v):"m" (*addr)); }
    _v
}
...
#[inline]
fn get_fs_long(addr: *const u32) -> u32 {
    let _v: u32;
...
#[inline]
fn put_fs_byte(val: u8, addr: *mut u8) {
    unsafe { llvm_asm!("movb %0,%%fs:%1"::"r" (val),"m" (*addr)); }
}

#[inline]
fn put_fs_word(val: u16, addr: *mut u16) {
    unsafe { llvm_asm!("movw %0,%%fs:%1"::"r" (val),"m" (*addr)); }
}

#[inline]
fn put_fs_long(val: u32, addr: *mut u32) {
